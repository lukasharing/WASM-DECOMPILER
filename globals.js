...
function functype(data, i){
    ...
    return {
        value: {rt1: rt1.value, rt2: rt2.value},
        bytes: pointer - i
    };
}

const IMPORTTYPE = {
    typeidx:  { value: { hex: 0x0, type: "type"   } },
    tableidx: { value: { hex: 0x1, type: "table"  } },
    memidx:   { value: { hex: 0x2, type: "mem"    } },
    globalidx:   { value: { hex: 0x3, type: "global" } },
};


const funcidx   = (data, i) => u32(data, i);
const labelidx  = (data, i) => u32(data, i);
const typeidx   = (data, i) => u32(data, i);
const tableidx  = (data, i) => u32(data, i);
const memidx    = (data, i) => u32(data, i);
const localidx = (data, i) => u32(data, i);
const globalidx = (data, i) => u32(data, i);
...
function importdesc(data, i){
    ...
    switch(magic.value){
        case IMPORTTYPE.typeidx.value.hex: 
            ...
            importdesc_result = Object.assign({type: x.value}, IMPORTTYPE.typeidx.value);
        break;
        case IMPORTTYPE.tableidx.value.hex: 
            const tt = tabletype(data, pointer);
            pointer += tt.bytes;

            importdesc_result = Object.assign({table: tt.value}, IMPORTTYPE.tableidx.value);
        break;
        case IMPORTTYPE.memidx.value.hex:
            const mt = memtype(data, pointer);
            pointer += mt.bytes;

            importdesc_result = Object.assign({mem: mt.value}, IMPORTTYPE.memidx.value);
        break;
        case IMPORTTYPE.globalidx.value.hex:

            const gt = globaltype(data, pointer);
            pointer += gt.bytes;

            importdesc_result = Object.assign({global: gt.value}, IMPORTTYPE.globalidx.value);
        break;
...
function import_wasm(data, i){
    ...
    return {
        value: {mod: mod.value, nm: nm.value, d: d.value},
        bytes: pointer - i
    };
}
...
function tabletype(data, i){
    ...
    return {
        value: {et: et.value, lim: lim.value},
        bytes: pointer - i
    };
}
...
function mut(data, i){
    ...
    switch(magic.value){
        ...
        default: throw `MUTTYPE ${toHex(magic.value)} Not found, expected ${Object.entries(IMPORTTYPE).map(e => `${e[0]} (${toHex(e[1].value.hex)})`).join(" or ")}`;
    }
...
function instr(data, i){
    ...
    switch(magic.value){
        ...
        case INSTRYPE.call.value.hex:
            ...
            instr_result = Object.assign({ x: x_call.value }, INSTRYPE.call.value);
        ...
        case INSTRYPE.call_indirect.value.hex:
            ...
            instr_result = Object.assign({ x: x_call_indirect.value }, INSTRYPE.call_indirect.value);
...
const EXPORTTYPE = {
    funcidx:   { value: { hex: 0x0, type: "func"   } },
    tableidx:  { value: { hex: 0x1, type: "table"  } },
    memidx:    { value: { hex: 0x2, type: "memory"    } },
    globalidx: { value: { hex: 0x3, type: "global" } },
};
...
module.exports = { u32, i32, ibyte, ubyte, vec, functype, import_wasm, export_wasm, typeidx, table, mem, global, elem, code, data_wasm };